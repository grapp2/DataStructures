...
public class DoubleArraySeq implements Cloneable
{
    ...
    {
        if (manyItems + 1 > getCapacity())
        {
            double[] copy = new double[data.length * 2 + 1];
...
//        System.out.println(str);
        ...
        throws NullPointerException, OutOfMemoryError
    {
        ...
        {
            double[] copy = new double[getCapacity() + addend.getCapacity()];
            for (int i = 0; i < manyItems; i++)
            {
                copy[i] = data[i];
            }
    ...
    {
        DoubleArraySeq copy = new DoubleArraySeq(getCapacity());
        for (int i = 0; i < manyItems; i++)
        {
            copy.data[i] = data[i];
        }
    ...
    public static DoubleArraySeq concatenation(DoubleArraySeq s1,
        ...
        throws NullPointerException, OutOfMemoryError
    {
        DoubleArraySeq copy = new DoubleArraySeq(s1.getCapacity() 
            + s2.getCapacity());

        for (int i = 0; i < s1.size(); i++)
        {
            copy.addAfter(s1.data[i]);
        }
        for (int i = 0; i < s2.size(); i++)
        {
            copy.addAfter(s2.data[i]);
        }
        ...
        throws IllegalStateException
    {
        ...
        else if (isCurrent() && currentIndex == manyItems)
        {
            currentIndex = -1;
            manyItems--;
        }
    ...
    public void trimToSize()
        throws OutOfMemoryError
    {
        DoubleArraySeq copy = new DoubleArraySeq(manyItems);
        for (int i = 0; i < manyItems; i++)
        {
            copy.data[i] = data[i];
        }
     ...
     *         otherwise.
     */
    public boolean equals(DoubleArraySeq other)
    {
        int count = 0;
        if (other.manyItems == manyItems)
        {
            for (int i = 0; i < manyItems; i++)
            {
        ...
        return (count == manyItems && other.currentIndex == currentIndex);
    }
    ...
    public static void main(String[] args)
    {
        ...
        DoubleArraySeq e = new DoubleArraySeq();
        e.addBefore(12);
        e.addBefore(3);
